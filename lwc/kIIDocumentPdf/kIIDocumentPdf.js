...
import getKIIDocumentPDF from '@salesforce/apex/RenderKIIDocumentPDFActionService.getKIIDocumentPDF';
import fetchCustomMetaDataType from '@salesforce/apex/RenderKIIDocumentPDFActionService.fetchCustomMetaDataType';
...
export default class KIIDocumentPdf extends LightningElement {
    @api recordId;
    ...
    @track kiiPdfSuccessMessage;
    @track kiiPdfErrorMessage;
    @track kiiCodeError;
    ...
    renderedCallback() {
        ...
        if (this.retriveRecordId && this.recordId) {
            ...
            this.SamplePDFURL = '/apex/fe_FundInfo__GenerateKIIDocumentPDFHelper?data=';
            getKIIDocumentPDF({portfolioHoldingId: this.recordId}) 
                .then(result =>{
                    ...
                    if (result.isSuccessForValidate && result.data != 'error' && result.isSuccess) {
                        ...
                        this.showNotification(result.kiiErrorMessage, 'error', 'sticky');
                ...
                .catch(error =>{
                    ...
                    this.isLoading = false;
                    this.retriveRecordId = false;
                    this.showNotification(this.kiiPdfErrorMessage, 'error', 'sticky');
                    this.dispatchEvent(new CloseActionScreenEvent());
                })
        }
    }
    ...
    fetchCustomMetaDataForToastMessage() {
        fetchCustomMetaDataType()
            .then(result => {
                console.log(result);
                this.kiiPdfErrorMessage = result.Fe_FundInfo__KII_Error_Message__c;
                this.kiiPdfSuccessMessage = result.Fe_FundInfo__KII_Success_Message__c;
                ...
                this.kiiCodeError = result.Fe_FundInfo__Does_Not_Have_KII_Code_Error_Message__c;
            })
            .catch(error => {
                console.log(error);
            })
