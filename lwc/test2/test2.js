...
export default class Test2 extends LightningElement {
    ...
    renderedCallback() {
        ...
        this.template.querySelector('lightning-spinner').appendChild(style1);
        ...
        if (this.retriveRecordId && this.recordId) {
            ...
            this.SamplePDFURL = '/apex/fe_FundInfo__GenerateAdvisorFactSheetPDF?data=';
            getAdvisorFactSheetPDF({ portfolioHoldingId: this.recordId })
                .then(result => { 
                    ...
                    if (result.isSuccessForValidate && result.data != 'error' && result.isSuccess) {
                        ...
                        this.showNotification(result.advisorFactSheetErrorMessage , 'warning', 'sticky');
    ...
    fetchCustomMetaDataForToastMessage() {
        fetchCustomMetaDataType()
            .then(result => {
                console.log(result);
                this.advisorFactSheetErrorMessage = result.Fe_FundInfo__AdvisorFactSheet_Error_Message__c;
