...
import getProductHighlightSheetPDF from '@salesforce/apex/ProductHighlightSheetActionService.getProductHighlightSheetPDF';
import fetchCustomMetaDataType from '@salesforce/apex/ProductHighlightSheetActionService.fetchCustomMetaDataType';
...
export default class ProductHighlightSheetPdf extends LightningElement {
    @api recordId;
    ...
    @track productHighlightSheetPdfSuccessMessage;
    @track productHighlightSheetPdfErrorMessage;
    @track productHighlightSheetCodeError;
    ...
    renderedCallback() {
        ...
        if (this.retriveRecordId && this.recordId) {
            ...
            this.SamplePDFURL = '/apex/fe_FundInfo__GenerateProductHighlightSheetPDFHelper?data=';
            getProductHighlightSheetPDF({ portfolioHoldingId: this.recordId }) 
                .then(result =>{
                    ...
                    if (result.isSuccessForValidate && result.data != 'error' && result.isSuccess) {
                        ...
                        this.showNotification(result.productHighlightSheetErrorMessage, 'error', 'sticky');
                ...
                .catch(error =>{
                    ...
                    this.isLoading = false;
                    this.retriveRecordId = false;
                    this.showNotification(this.productHighlightSheetPdfErrorMessage, 'error', 'sticky');
                    this.dispatchEvent(new CloseActionScreenEvent());
                })
        }
    }
    ...
    fetchCustomMetaDataForToastMessage() {
        fetchCustomMetaDataType()
            .then(result => {
                console.log(result);
                this.productHighlightSheetPdfErrorMessage = result.Fe_FundInfo__ProductHighlightSheet_Error_Message__c;
                this.productHighlightSheetPdfSuccessMessage = result.Fe_FundInfo__ProductHighlightSheet_Success_Message__c;
                ...
                this.productHighlightSheetCodeError = result.Fe_FundInfo__Does_Not_Have_PHS_Code_Error_Message__c;
            })
            .catch(error => {
                console.log(error);
            })
