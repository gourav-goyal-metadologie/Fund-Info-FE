...
import getProviderFactSheetPDF from '@salesforce/apex/RenderProviderFactSheetPDFActionService.getProviderFactSheetPDF';
import fetchCustomMetaDataType from '@salesforce/apex/RenderProviderFactSheetPDFActionService.fetchCustomMetaDataType';
...
export default class ProviderFactsheetPdf extends LightningElement {
    @api recordId;
    ...
    @track providerFactSheetPdfSuccessMessage;
    @track providerFactSheetPdfErrorMessage;
    @track pfsCodeErrorMessage;
    ...
    renderedCallback() {
        ...
        if (this.retriveRecordId && this.recordId) {
            ...
            this.SamplePDFURL = '/apex/fe_FundInfo__GenerateProviderFactSheetPDFHelper?data=';
            getProviderFactSheetPDF({ portfolioHoldingId: this.recordId })
                .then(result => { 
                    ...
                    if (result.isSuccessForValidate && result.data != 'error' && result.isSuccess) {
                        ...
                        this.showNotification(result.providerFactSheetErrorMessage, 'error', 'sticky');
                ...
                .catch(error =>{
                    ...
                    this.isLoading = false;
                    this.retriveRecordId = false;
                    this.showNotification(this.providerFactSheetPdfErrorMessage, 'error', 'sticky');
                    this.dispatchEvent(new CloseActionScreenEvent());
                })
        }
    }
    ...
    fetchCustomMetaDataForToastMessage() {
        fetchCustomMetaDataType()
            .then(result => {
                console.log(result);
                this.providerFactSheetPdfErrorMessage = result.Fe_FundInfo__ProviderFactSheet_Error_Message__c;
                this.providerFactSheetPdfSuccessMessage = result.Fe_FundInfo__ProviderFactSheet_Success_Message__c;
                ...
                this.pfsCodeErrorMessage = result.Fe_FundInfo__Does_Not_Have_PFS_Code_Error_Message__c
            })
            .catch(error => {
                console.log(error);
            })
